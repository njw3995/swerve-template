 ...
 */
public class SecondOrderSwerveDrive extends SwerveDrive
{
  ...
  /**
   * Creates a new swerve drivebase subsystem. Robot is controlled via the {@link SecondOrderSwerveDrive#drive} method, or via the
   * {@link SecondOrderSwerveDrive#setRawModuleStates} method. The {@link SecondOrderSwerveDrive#drive} method incorporates kinematics-- it
   * takes a translation and rotation, as well as parameters for field-centric and closed-loop velocity control.
   * {@link SecondOrderSwerveDrive#setRawModuleStates} takes a list of SwerveModuleStates and directly passes them to the modules.
      ...
      Translation2d translation, double rotation, boolean fieldRelative, boolean isOpenLoop, boolean headingCorrection)
  {
    ...
    if (Constants.SwerveConstants.headingCorrection)
    {
      ...
      {
        if (Math.sqrt(velocity.vxMetersPerSecond * velocity.vxMetersPerSecond 
                    + velocity.vyMetersPerSecond * velocity.vyMetersPerSecond) < 0.05 * swerveDriveConfiguration.maxSpeed)
        {
          velocity.omegaRadiansPerSecond =
              swerveController.headingCalculate(lastHeadingRadians, getYaw().getRadians(), true);
        }
        else
        {
          velocity.omegaRadiansPerSecond =
              swerveController.headingCalculate(lastHeadingRadians, getYaw().getRadians(), false);
        }
      } else
      {
   ...
   * {@link SecondOrderSwerveDrive#setRawModuleStates(SwerveModuleState2[], boolean)} function and
   ...
   *                                {@link SecondOrderSwerveDrive#replaceSwerveModuleFeedforward(SimpleMotorFeedforward)}
   */
  ...
  /**
   * Helper function to get the {@link SecondOrderSwerveDrive#swerveController} for the {@link SecondOrderSwerveDrive} which can be used to
   ...
   *
   * @return {@link SwerveController} for the {@link SecondOrderSwerveDrive}.
   */
  ...
  {
    swerveTranslationScalar = scalar;
  }
  ...
  {
    if (!SwerveDriveTelemetry.isSimulation) {
      Rotation3d current = imu.getRawRotation3d();
      imu.setOffset(new Rotation3d(current.getX(), current.getY(), current.getZ() + Math.toRadians(offset)));
    } else {
      simIMU.setAngle(offset);
    }
    lastHeadingRadians = 0;
    resetOdometry(new Pose2d(getPose().getTranslation(), getPose().getRotation().plus(new Rotation2d(Math.toRadians(offset)))));
  }
  ...
  @Override
  protected void setRawModuleStates(SwerveModuleState[] desiredStates, boolean isOpenLoop) {
