 ...
 */
public class FirstOrderSwerveDrive extends SwerveDrive
{

  /**
   * Swerve Kinematics object utilizing first order kinematics.
   */
  ...
  /**
   * Creates a new swerve drivebase subsystem. Robot is controlled via the {@link FirstOrderSwerveDrive#drive} method, or via the
   * {@link FirstOrderSwerveDrive#setRawModuleStates} method. The {@link FirstOrderSwerveDrive#drive} method incorporates kinematics-- it
   * takes a translation and rotation, as well as parameters for field-centric and closed-loop velocity control.
   * {@link FirstOrderSwerveDrive#setRawModuleStates} takes a list of SwerveModuleStates and directly passes them to the modules.
      ...
      Translation2d translation, double rotation, boolean fieldRelative, boolean isOpenLoop)
  {
    ...
    if (Constants.SwerveConstants.HEADING_CORRECTION)
    {
      ...
      {
        boolean useSnap = Math.sqrt(velocity.vxMetersPerSecond * velocity.vxMetersPerSecond 
                                  + velocity.vyMetersPerSecond * velocity.vyMetersPerSecond) < 0.1 * swerveDriveConfiguration.maxSpeed;

        velocity.omegaRadiansPerSecond =
                 swerveController.headingCalculate(lastHeadingRadians, getYaw().getRadians(), useSnap);
      } 
      else
      {
    ...
    velocity.vxMetersPerSecond     = velocity.vxMetersPerSecond 
                                   * swerveTranslationScalar;

    velocity.vyMetersPerSecond     = velocity.vyMetersPerSecond 
                                   * swerveTranslationScalar;

    velocity.omegaRadiansPerSecond = velocity.omegaRadiansPerSecond 
                                   * swerveRotationScalar;
